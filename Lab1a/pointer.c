 ...
 *
 * Name(s): Jaehyeon Ahn 
 * NetID(s): hereaj@cs.washington.edu
 *
...
int int_size() {
  ...
  int_ptr1 = &int_array;
  int_ptr2 = int_ptr1 + 1;
  diff = (char*)int_ptr2 - (char*)int_ptr1;
...
int double_size() {
  ...
  diff = (char*)doub_ptr2 - (char*)doub_ptr1;
...
int within_same_block(int* ptr1, int* ptr2) {
  int ptr1Above64bits = (int)ptr1 >> 6;
  int ptr2Above64bits = (int)ptr2 >> 6;
  int result = !(ptr1Above64bits ^ ptr2Above64bits);
  return result;
}
...
int within_array(int* int_array, int size, int* ptr) {
  int* bit_index;
  int* end_address = int_array + size;
  int result = 0;

  for (bit_index = int_array; !(bit_index == end_address); bit_index++) {
      if (bit_index == ptr) {
        result = 1;
        break;
      }
