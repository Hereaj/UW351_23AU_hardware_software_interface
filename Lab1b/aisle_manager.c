...
unsigned short get_section(unsigned long* aisle, int index) {
  unsigned long aisle_retrive = *aisle;
  unsigned short section = (unsigned short)((aisle_retrive >> (index * SECTION_SIZE)) & SECTION_MASK);
  return section;
}
...
unsigned short get_id(unsigned long* aisle, int index) {
  unsigned short section = get_section(aisle, index);
  unsigned short item_id = ((section & ID_MASK) >> NUM_SPACES);
  return item_id;
}
...
void set_section(unsigned long* aisle, int index, unsigned short new_section) {
  unsigned short* section_address = (unsigned short*)aisle + index;
  *section_address = *section_address & ~SECTION_MASK;
  *section_address = *section_address | new_section;
}
...
void set_spaces(unsigned long* aisle, int index, unsigned short new_spaces) {
  if (new_spaces > SPACES_MASK) {
    ...
    unsigned short* section_address = (unsigned short*)aisle + index;
    *section_address &= ~SPACES_MASK;
    *section_address |= new_spaces;
  }
}
...
void set_id(unsigned long* aisle, int index, unsigned short new_id) {
  unsigned short id_limit = ID_MASK >> NUM_SPACES;
  if (new_id > id_limit) {
    return;
  } else {
    unsigned short* section_address = (unsigned short*)aisle + index;
    unsigned short new_id_aligned = new_id << NUM_SPACES;
    *section_address &= ~ID_MASK;
    *section_address |= new_id_aligned;
  } 
}
...
void toggle_space(unsigned long* aisle, int index, int space_index) {
  unsigned short* section_address = (unsigned short*)aisle + index;
  *section_address = *section_address ^ (1 << space_index);
}
...
void add_items(unsigned long* aisle, int index, int n) {
  unsigned short* section_address = (unsigned short*)aisle + index;
  int empty_spaces = NUM_SPACES - num_items(aisle, index);
  if (n >= empty_spaces) {
    *section_address |= SPACES_MASK;
  } else {
    for (int i = 0; i < n; i++) {
      int space_index = 0;
      unsigned short section = *section_address;
      while (section & (1 << space_index)) {
        space_index++;
      }
...
void remove_items(unsigned long* aisle, int index, int n) {
  ...
  if (n > num_items(aisle, index)) {
    *section_address = *section_address & ID_MASK;
  } else {
    for (int i = 0; i < n; i++) {
      int space_index = 0;
      unsigned short section = *section_address;
...
void rotate_items_left(unsigned long* aisle, int index, int n) {
  ...
  int i;
  int newN = n % 10;
  int spaces_MSB_check = (1 << 9);
  unsigned short id = get_id(aisle, index);
  unsigned short spaces = get_spaces(aisle, index);
  for (i = 0; i < newN; i++){
    if (spaces & spaces_MSB_check){
      spaces = ((spaces << 1) & SPACES_MASK) | 1; 
    } else {
      spaces = (spaces << 1) & SPACES_MASK;
    }
  }
  *section_address = (id << NUM_SPACES) | spaces;
}
...
void rotate_items_right(unsigned long* aisle, int index, int n) {
  ...
  int i;
  int newN = n % 10;
  int spaces_LSB_check = 1;
  unsigned short id = get_id(aisle, index);
  unsigned short spaces = get_spaces(aisle, index);
  for (i = 0; i < newN; i++){
    if (spaces & spaces_LSB_check){
      spaces = ((spaces >> 1) & SPACES_MASK) | (1 << 9);
    } else {
      spaces = (spaces >> 1) & SPACES_MASK;
    }
  }
