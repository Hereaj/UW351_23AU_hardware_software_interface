...
void* mm_malloc(size_t size) {
  ...
  // Find free block of req_size
  ptr_free_block = search_free_list(req_size);
  while (ptr_free_block == NULL) {
    request_more_space(1 << 14);
    ptr_free_block = search_free_list(req_size);
  }

  /*
   * Check free_block_size, req_size, and alignment requirements
   * Split the block in the necessary case.
   * If block splits, re-format created free block.
   * Adding header, setting bits, adding footer. 
   */

  // Declear block_info pointer to split the block.
  block_info* split_block;

  block_size = SIZE(ptr_free_block->size_and_tags);
  // The case of spliting
  if (block_size - req_size >= MIN_BLOCK_SIZE) {
    // Set size and new block's tags
    split_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    block_size -= req_size;
    split_block->size_and_tags = block_size | TAG_PRECEDING_USED;
    // Make used bit off
    split_block->size_and_tags &= ~0 << 1;

    // Set footer.
    // Current ptr + block_size - (1 * WORD_SIZE) = boundary
    *((size_t*) UNSCALED_POINTER_ADD(split_block, block_size - WORD_SIZE)) = split_block->size_and_tags;
    insert_free_block(split_block);
    // Set size of ptr_free_block to exclude size of new Block.
    block_size = req_size;
  } else {
    // Set the next block's preceding tag to 1 in the case of non-splitting.
    *((size_t*) UNSCALED_POINTER_ADD(ptr_free_block, SIZE(ptr_free_block->size_and_tags))) |= TAG_PRECEDING_USED;
  }
  ...
  // If the preceding block is currently used, set lower two bits.
  if (preceding_block_use_tag) {
    ptr_free_block->size_and_tags = block_size | (TAG_PRECEDING_USED + TAG_USED);
  } else {
    // Else, set used bit.
    ptr_free_block->size_and_tags = block_size | TAG_USED;
  }
  ...
  if (ptr_free_block == NULL) {
    ...
    // Else return one WORD_SIZE after start of block.
...
void mm_free(void* ptr) {
  ...
  // Set block_to_free pointer to include header.
  ...
  // Declear the local variable for bit-masking.
  size_t bit_mask;

  // Set header.
  // Used bit checking.
  bit_mask = ~0 << 1;
  block_to_free->size_and_tags &= bit_mask;

  // Copy header into footer.
  *((size_t*) UNSCALED_POINTER_ADD(block_to_free, payload_size)) = block_to_free->size_and_tags;

  // Check preceding use tag for following block.
  bit_mask = (~0 << 2) | 1;
  following_block->size_and_tags &= bit_mask;
